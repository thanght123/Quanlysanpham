...
function addChiTietToWeb(p) {
    ...
    // Đổi title
    document.title = p.TenSP + ' - Shoes Shop';
...
function getDetailPromo(sp) {
    return sp.MoTa;
    // switch (sp.MaKM.LoaiKM) {
    //     case 'giamgia':
    //         var span = `<span style="font-weight: bold">` + Number(sp.MaKM.GiaTriKM).toLocaleString() + `</span>`;
    //         return `Khách hàng sẽ được giảm ` + span + `₫ khi mua sản phẩm`;

    //     case 'moiramat':
    //         return `Khách hàng sẽ được thử máy miễn phí tại cửa hàng. Có thể đổi trả lỗi trong vòng 2 tháng.`;

    //     case 'giareonline':
    //         var del = Number(p.DonGia) - Number(p.MaKM.GiaTriKM);
