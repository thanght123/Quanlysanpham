...
window.onload = function() {
    ...
    var tags = ["Adidas", "Bitis", "Converse", "Nike", "Puma", "Vans"];
    ...
    addPricesRange(400000, 700000);
    addPricesRange(700000, 1000000);
    addPricesRange(1000000, 0);
...
function hienThiKhungSanPhamMacDinh() {
    ...
    addKhungSanPham('GIÁ RẺ CHO MỌI NHÀ', green, ['price=0-300000', 'sort=DonGia-asc', 'page=0'], soLuong);
}
...
function priceToString(min, max) {
    if (min == 0) return 'Dưới ' + parseInt(max).toLocaleString() + ' vnđ'; //parseInt(max).toLocaleString()
    if (max == 0) return 'Trên ' + parseInt(min).toLocaleString() + ' vnđ';
    return 'Từ ' + parseInt(min).toLocaleString() + ' - ' + parseInt(max).toLocaleString() + ' vnđ';
}
