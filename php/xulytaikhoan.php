<?php
	...
	function dangNhap() {
		...
		$maquyen = "SELECT MaQuyen FROM nguoidung WHERE TaiKhoan='$taikhoan' AND MatKhau='$matkhau'";
		
		$sql = "SELECT * FROM nguoidung WHERE TaiKhoan='$taikhoan' AND MatKhau='$matkhau' AND (MaQuyen=1 OR MaQuyen=2 OR MaQuyen=3) AND TrangThai=1";
	...
	function themNV() {
		...
		$nvBUS = new NguoiDungBUS();
		die (json_encode($nvBUS->add_new($nvAddArr)));
	}
	...
	function themKH() {
		...
		$khBUS = new NguoiDungBUS();
		die (json_encode($khBUS->add_new($khAddArr)));
	}
?>